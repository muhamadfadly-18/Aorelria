...
function buildMontageScene() {
  ...
  slots.forEach((slotName, index) => {
    ...
    card.appendChild(createMontageMediaElement(montageMedia[index % montageMedia.length], index));
    montageTrack.appendChild(card);
  });
}

function rotateMontageScene() {
  const cards = montageTrack.querySelectorAll(".montage-card");

  cards.forEach((card, cardIndex) => {
    const currentIndex = Number(card.dataset.mediaIndex || cardIndex);
    const nextIndex = (currentIndex + 5) % montageMedia.length;
    card.dataset.mediaIndex = String(nextIndex);
    card.innerHTML = "";
    card.appendChild(createMontageMediaElement(montageMedia[nextIndex], nextIndex));
  });
}

function startMontageScene() {
  rotateMontageScene();
  montageRotationId = window.setInterval(rotateMontageScene, 1200);
}

function stopMontageScene() {
  if (montageRotationId) {
    window.clearInterval(montageRotationId);
    montageRotationId = null;
  }
}
