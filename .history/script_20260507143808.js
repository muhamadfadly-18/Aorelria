...
const memoryImage = document.getElementById("popupImage");
const memoryTitle = document.getElementById("popupTitle");
const memorySubtitle = document.getElementById("popupSubtitle");
const memoryText = document.getElementById("popupText");
...
function burstMemoryParticles() {
  const rect = memoryParticles.getBoundingClientRect();
...
fruits.forEach((fruit) => {
  fruit.addEventListener("click", () => {
    ...
    fruit.classList.add("opened");
    fruit.style.opacity = "0.24";
    fruit.style.transform = "scale(1.15)";
...
function checkCollision() {
  ...
  fruits.forEach((fruit) => {
    const orbRect = fruit.getBoundingClientRect();
    ...
    if (hit && !fruit.classList.contains("opened")) {
      fruit.classList.add("opened");
      const memoryIndex = Number(fruit.dataset.memory);
...
const fruits =
  document.querySelectorAll(".memory-fruit");
