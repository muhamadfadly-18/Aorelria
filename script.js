...
function initExperience() {
  ...
  window.removeEventListener("touchstart", initExperience);
  window.removeEventListener("click", initExperience);
}
...
const calculatorMemories = {
  ...
  8: {
    image: "assets/images/memory-photo-07.jpeg",
  ...
  9: {
    image: "assets/images/memory-photo-10.jpeg",
...
calcButtons.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    if (value === "C") {
      ...
      calculatorScreen.textContent = "Terimakasih Sayang ♡";

      const thanks = document.createElement("div");

      thanks.className = "love-thanks";

      thanks.innerHTML = `
    <div class="love-thanks__text">
      <h1>Terimakasih Sayang ♡</h1>
      <p>
        Makasih sudah jadi bagian dari cerita terindah ini.
        Semoga semua kenangan kecil kita tetap hidup
        di hati selamanya ✨
      </p>
    </div>
  `;
...
const popupCanvas = document.getElementById("popupStars");
...
window.addEventListener("mousemove", (e) => {
  popupMouse.x = e.clientX;
  popupMouse.y = e.clientY;
});
...
class PopupStar {
  ...
  update() {
    const dx = popupMouse.x - this.x;

    const dy = popupMouse.y - this.y;
    ...
    if (
      this.x < 0 ||
      this.x > popupCanvas.width ||
      this.y < 0 ||
      this.y > popupCanvas.height
    ) {
      this.reset();
    }
  }
  ...
  draw() {
    ...
    popupCtx.arc(this.x, this.y, this.size, 0, Math.PI * 2);

    popupCtx.fillStyle = "rgba(255,255,255,.9)";

    popupCtx.shadowBlur = 12;

    popupCtx.shadowColor = "rgba(255,255,255,.8)";
...
for (let i = 0; i < 90; i++) {
  popupParticles.push(new PopupStar());
}

function connectPopupStars() {
  for (let a = 0; a < popupParticles.length; a++) {
    for (let b = a; b < popupParticles.length; b++) {
      const dx = popupParticles[a].x - popupParticles[b].x;

      const dy = popupParticles[a].y - popupParticles[b].y;
      ...
      if (dist < 120) {
        ...
        popupCtx.strokeStyle = `rgba(255,255,255,${0.12 - dist / 1000})`;

        popupCtx.lineWidth = 1;

        popupCtx.moveTo(popupParticles[a].x, popupParticles[a].y);

        popupCtx.lineTo(popupParticles[b].x, popupParticles[b].y);
...
function animatePopupStars() {
  ...
  requestAnimationFrame(animatePopupStars);
}
...
function createLoveFireworks() {
  for (let i = 0; i < 12; i++) {
    ...
    firework.className = "love-firework";
